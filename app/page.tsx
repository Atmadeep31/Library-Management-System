...
export default async function HomePage() {
  ...
  // Unified background parameters to cover the screen viewport perfectly
  const sharedBackground = {
    backgroundImage: `url('/Vibrant Animated University Campus Banner.png')`,
  ...
  if (!userId) {
    return (
      <div 
        style={sharedBackground} 
        className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-6 -mt-6"
      >
        {/* Dark Frosted Glass Landing Panel */}
        <div className="max-w-3xl w-full text-center bg-slate-900/80 backdrop-blur-xs border border-white/10 rounded-2xl p-10 md:p-16 shadow-2xl">
  ...
  return (
    <div 
      style={sharedBackground} 
      className="min-h-[calc(100vh-4rem)] p-6 -mt-6 flex items-center justify-center"
    >
      {/* Semi-transparent structural card layout container */}
      <div className="max-w-5xl w-full bg-slate-900/85 backdrop-blur-xs border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        {/* Informative Grid Summary Panels (Converted to premium dark cards for contrast) */}
